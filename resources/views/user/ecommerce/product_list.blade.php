                    ...
                    <div class="position-relative rounded-top overflow-hidden">
                      ...
                      <div class="swiper theme-slider" data-swiper='{"autoplay":true,"autoHeight":true,"spaceBetween":5,"loop":true,"loopedSlides":5,"navigation":{"nextEl":".swiper-button-next","prevEl":".swiper-button-prev"}}'>
                        <div class="swiper-wrapper">
                              ...
                              <div class="swiper-slide"><a class="d-block" href="../../../app/e-commerce/product/product-details.html"><img class="rounded-top img-fluid" src="{{ asset('/product_images/'. $image) }}" alt="" /></a></div>
                              @endforeach
                            </div>
                      ...
                      @else
                      <a class="d-block" href="../../../app/e-commerce/product/product-details.html"><img class="img-fluid rounded-top" src="{{ asset('/product_images/' . $product->images) }}" alt="" />
                      @endif
                      </a><span class="badge rounded-pill bg-success position-absolute mt-2 me-2 z-2 top-0 end-0">New</span>
                    </div>
                  <div class="p-3">
                    <h5 class="fs-9"><a class="text-1100" href="../../../app/e-commerce/product/product-details.html">{{ $product->name }}</a></h5>
                    <p class="fs-10 mb-3"><a class="text-500" href="#!">{{ $product->category->name }} {{ isset($product->subcategory) ? " / ". $product->subcategory->name : ''  }}</a></p>
                    <h5 class="fs-md-7 text-warning mb-0 d-flex align-items-center mb-3"> ${{ $product->product_price }}
                      {{-- <del class="ms-2 fs-10 text-500">${{ $product->product_price }} </del> --}}
                    </h5>
                    {{-- <p class="fs-10 mb-1">Shipping Cost: <strong>${{ $product->shipping_cost }}</strong></p> --}}
                    <p class="fs-10 mb-1">Stock: <strong class="{{ $product->product_stock > 0 ? 'text-success' : 'text-danger' }}">{{ $product->product_stock > 0 ? 'Available' : 'Out of Stock' }}</strong></p>
                  </div>
                </div>
